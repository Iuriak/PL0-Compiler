...
void Parser::emit(string op, string arg1, string arg2, string result) {
    IR.push_back({op, arg1, arg2, result});
};

string Parser::getTempVar(){
...
void Parser::conditionStatement(){
    ...
    emit(orp, arg1, arg2, temp_IF);

    auto label2 = IR.size();
    emit("JUMPFALSE", temp_IF, "_", "M");
    // token 此时应该指向 THEN
...
void Parser::cyclicStatement(){
    ...
    emit(orp, arg1, arg2, temp_WHILE);

    auto label2 = IR.size();
    emit("JUMPFALSE", temp_WHILE, "_", "M");
    // token 指向 DO
    ...
    // IR[label1][3] = to_string(IR.size());
    statement();    // <语句>
    emit("JUMP", "_", "_", to_string(label1 + base_addr));
    IR[label2][3] = to_string(IR.size() + base_addr);
}
